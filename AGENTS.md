...
- Use `workflow quality-fix` when a finding maps to a safe next step like guidance scaffolding, EditMode test scaffolding, sandbox-scene creation, repairing `AudioListener` or `EventSystem` scene wiring, adding missing `CanvasScaler` or `GraphicRaycaster` components, scaffolding or wiring an Animator Controller, or repairing importer mismatches. By default it plans the next move; add `--apply` only for the bounded fixes that are explicitly supported.
