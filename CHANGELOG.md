...
- **safer `improve project` chat pass** — the offline Agent assistant now scopes test detection to files inside the Unity `Assets` tree instead of matching outer temp-folder names, so the safe improvement bundle no longer skips test scaffolding just because the workspace sits under `.tmp-tests`. The reply also reports a before/after quality-score delta instead of only the final score.
