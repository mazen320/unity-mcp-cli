...
- **bounded CharacterController repair in the offline assistant** — the same live `improve project` flow can now add a `CharacterController` to one clear likely player object when the physics setup has no movement body, and it explicitly refuses to guess when multiple player-like candidates exist.
- **top-level `workflow improve-project` command** — the CLI now has a first-class safe improvement pass that mirrors the bounded assistant bundle instead of leaving that behavior trapped inside the Unity Agent tab. Offline it writes missing guidance plus EditMode smoke-test scaffolding when the Unity Test Framework is present; with `--port` it also runs sandbox-scene creation, disposable probe cleanup, `AudioListener` repair, `EventSystem` repair/normalization, missing `CanvasScaler` / `GraphicRaycaster` fixes, and the bounded likely-player `CharacterController` repair. The result includes `baselineScore`, `finalScore`, `scoreDelta`, and explicit `applied` / `skipped` fix lists so demos and GitHub evidence can show exactly what changed.
- **embedded `improve project` workflow unification** — the Agent chat path now delegates `improve project` through the top-level `workflow improve-project` command instead of maintaining a second handwritten offline/embedded implementation. The reply still renders the same human-readable summary, but the CLI and Agent tab now share one improvement engine, one score delta, and one applied/skipped-fix surface.
- **selected File IPC project reuse for `improve-project`** — `workflow improve-project` now auto-reuses the currently selected Unity editor when its `projectPath` matches `PROJECT_ROOT`, even when `--port` is omitted. `workflow agent-chat <PROJECT_ROOT>` also seeds that explicit File IPC project into session state first, so embedded Agent-tab improvements can keep their live-scene repair bundle without needing a separate direct implementation.
- **markdown export for `workflow improve-project`** — the same safe improvement pass now supports `--markdown-file`, which writes a GitHub-friendly summary of the run with the project root, live/offline status, quality-score delta, and explicit applied/skipped fix lists. That makes one improvement run sufficient for both JSON automation and human-readable benchmark/update evidence.
...
- **physics lens and profile** — added a dedicated `physics` expert lens plus a matching `physics` developer profile. It focuses on collider coverage, Rigidbody ownership, CharacterController gaps, and scenes that look playable but still have no collision foundation.
- **bounded physics movement-body fix** — `workflow quality-fix --lens physics --fix player-character-controller --apply` can now add a `CharacterController` to the single clear likely player object in a live scene, and the workflow now enriches physics audits/fixes with hierarchy context instead of silently degrading to manual mode.
- **reusable disposable-object cleanup fix** — `workflow quality-fix --lens systems --fix disposable-cleanup --apply` can now remove obvious probe, fixture, temp, debug, or standalone scene objects through the reusable systems workflow, so benchmark cleanup and live scene hygiene are no longer assistant-only.
...
- **systems lens EventSystem conflict detail** — the same systems audit now includes duplicate EventSystem names in the finding detail and flags `EventSystem has conflicting UI input modules` when both legacy and Input System modules are active on the same object.
- **systems lens AudioListener detail** — the same systems audit now includes duplicate `AudioListener` object names plus the likely keep target, and missing-listener findings now name the likely camera target, so systems benchmarks explain the audio repair path instead of only reporting raw listener counts.
- **E2E harness reliability fix** — `test_full_e2e` no longer hides the broad mock-route coverage block inside the self-transition regression, and the heaviest mock-route / `workflow audit-advanced` tests now use in-process Click invocation where appropriate so the full E2E suite completes reliably on slower local machines.
